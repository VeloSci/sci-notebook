...
function updateFooter() {
  ...
  if (footer) footer.innerHTML = `<span>${cellCount} cells</span><span>v${versionHistory.count} versions</span><span>sci-notebook v0.6.2 — Vanilla JS</span>`;
}

function getEngine(): EditorEngine | null {
  return vanillaNotebook?.getEngine() ?? null;
}
