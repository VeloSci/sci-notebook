    ...
    <div v-if="showJson" class="json-modal-overlay" @click="showJson = false">
      <div class="json-modal" @click.stop>
        ...
        <div class="json-modal-actions">
          ...
          <button v-if="jsonContent && jsonContent.length > 10" class="app-btn" @click="navigator.clipboard.writeText(jsonContent)">Copy</button>
      ...
      <div class="sci-nb-presentation">
        <div class="sci-nb-slide">
          <div class="sci-nb-slide-content">
            <div
              v-for="cell in presentationEngine.getCurrentSlide()?.cells"
              :key="cell.id"
        ...
        <div class="sci-nb-presentation-controls">
          ...
          <span class="sci-nb-slide-number">{{ currentSlide + 1 }} / {{ presentationEngine.getSlideCount() }}</span>
          <button @click="nextSlide" :disabled="currentSlide >= presentationEngine.getSlideCount() - 1">Next →</button>
          <button @click="handleEndPresentation">✕ Exit</button>
        </div>
        <div class="sci-nb-progress-bar">
          <div class="sci-nb-progress-bar-fill" :style="{ width: ((currentSlide + 1) / presentationEngine.getSlideCount() * 100) + '%' }" />
        </div>
      </div>
    </div>
  </div>
