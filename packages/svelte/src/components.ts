/**
 * DOM-based components for Svelte adapter.
...
const FORMAT_ACTIONS = [
  ...
  { label: "🔗", title: "Link", wrap: ["[", "](url)"], prefix: "" },
...
export class SlashCommandMenu {
  ...
  private render() {
    ...
    const header = el("div", "sci-nb-slash-header");
    header.textContent = "Insert block";
    this.el.appendChild(header);
    this.filtered.forEach((cmd, i) => {
      const btn = el("button", `sci-nb-slash-item${i === this.selectedIndex ? " sci-nb-slash-item--active" : ""}`);
...
export function createTOCSidebar(engine: EditorEngine, container: HTMLElement, focusedCellId?: string | null): HTMLElement {
  const nav = el("nav", "sci-nb-toc");
  const title = el("div", "sci-nb-toc-title");
...
export class FindReplaceBar {
  ...
  private render() {
    ...
    const count = el("span", "sci-nb-find-count");
...
export function renderCellOutputs(outputs: ICellOutput[]): string {
  ...
  return `<div class="sci-nb-cell-outputs">${outputs.map((o, i) => `<div class="sci-nb-output sci-nb-output--${o.outputType}">${renderCellOutput(o)}</div>`).join("")}</div>`;
}
...
export class ChatSidebarPanel {
  ...
  private render() {
    this.el.innerHTML = "";
    // Header
    const header = el("div", "sci-nb-chat-header");
    ...
    // Messages
    const msgsDiv = el("div", "sci-nb-chat-messages");
    ...
    if (visible.length === 0) {
      ...
      for (const msg of visible) {
        const msgEl = el("div", `sci-nb-chat-msg sci-nb-chat-msg--${msg.role}`);
    ...
    if (this.loading) {
      const loadEl = el("div", "sci-nb-chat-msg sci-nb-chat-msg--assistant");
    ...
    // Input
    const inputDiv = el("div", "sci-nb-chat-input");
    const input = document.createElement("input");
...
export class AIRewritePanel {
  ...
  private render() {
    ...
    if (this.state === "prompt") {
      const div = el("div", "sci-nb-ai-rewrite-prompt");
      ...
      const row = el("div", "sci-nb-ai-rewrite-input-row");
      const input = document.createElement("input");
      ...
      div.appendChild(row);
      if (this.error) { const errEl = el("div", "sci-nb-ai-rewrite-error"); errEl.textContent = this.error; div.appendChild(errEl); }
      this.el.appendChild(div);
      requestAnimationFrame(() => input.focus());
    } else if (this.state === "loading") {
      this.el.innerHTML = '<div class="sci-nb-ai-rewrite-loading"><span>Rewriting...</span></div>';
    } else {
      const div = el("div", "sci-nb-ai-rewrite-preview");
      ...
      const actions = el("div", "sci-nb-ai-rewrite-actions");
...
export class AICellGeneratePanel {
  ...
  private render() {
    this.el.innerHTML = "";
    if (this.state === "prompt") {
      const div = el("div", "sci-nb-ai-generate-prompt");
      ...
      div.appendChild(ta);
      const actions = el("div", "sci-nb-ai-generate-actions");
      ...
      const div = el("div", "sci-nb-ai-generate-preview");
      div.innerHTML = `<div class="sci-nb-ai-generate-header"><span>Generated ${this.cells.length} cell${this.cells.length !== 1 ? "s" : ""}</span></div>`;
      const cellsDiv = el("div", "sci-nb-ai-generate-cells");
      for (const cell of this.cells) {
        const cellEl = el("div", "sci-nb-ai-generate-cell");
...
export class ImageResizeHandle {
  ...
  constructor(
    ...
    this.handleEl.className = "sci-nb-image-resize-handle sci-nb-image-resize-handle--se";
    this.handleEl.onmousedown = (e) => this.startDrag(e);
  ...
  private startDrag(e: MouseEvent) {
    ...
    this.startW = this.imgEl.offsetWidth;
    this.dragging = true;

    const onMove = (e: MouseEvent) => {
      const dx = e.clientX - this.startX;
      this.width = Math.max(50, this.startW + dx);
    ...
    const onUp = () => {
      ...
      if (this.width !== null) {
        ...
        const pct = Math.round((this.width / parentW) * 100);
