...
let vanillaNotebook: SciNotebookVanilla | null = null;
...
function buildApp() {
  ...
  header.innerHTML = `
    <h1>
      ...
      Sci-Notebook <span class="app-framework-badge">Vanilla JS</span>
  ...
  footer.innerHTML = `<span>${cellCount} cells</span><span>v${versionHistory.count} versions</span><span>sci-notebook v0.6.2 — Vanilla JS</span>`;
  app.appendChild(footer);

  vanillaNotebook = new SciNotebookVanilla({
    target: nbContainer,
...
function updateFooter() {
  ...
  if (f) f.innerHTML = `<span>${cellCount} cells</span><span>v${versionHistory.count} versions</span><span>sci-notebook v0.6.2 — Vanilla JS</span>`;
}

function getEngine(): EditorEngine | null { return vanillaNotebook?.getEngine() ?? null; }
function toggleTheme() { theme = theme === "light" ? "dark" : "light"; vanillaNotebook?.setTheme(theme); app.setAttribute("data-app-theme", theme); const btn = document.getElementById("btn-theme"); if (btn) { btn.textContent = theme === "dark" ? "☀ Light" : "🌙 Dark"; btn.classList.toggle("app-btn--active", theme === "dark"); } }
